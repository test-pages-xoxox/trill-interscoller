var tools = {
    ...
    rollTextField: function (what,how) {
        how = how || {};
        if(what.txt && what.container) tools.rollText(what.container, what.txt, how.horizontal, how.infinite, how.noBreak, how.stopOnEnd, how.speed, how.speedMultiplier, how.customizeSpeed, how.widthCorrection)
    },
    rollText: function (container, txt, horizontal, infinite, noBreak, stopOnEnd, speed, speedMultiplier, customizeSpeed, widthCorrection) {
        horizontal = horizontal === undefined ? true : horizontal;
		widthCorrection = widthCorrection === undefined ? 0 : widthCorrection;
        infinite = infinite === undefined ? true : infinite;
        speed = (speed || (horizontal ? 4500 : 1000)) * (speedMultiplier===undefined ? 1 : speedMultiplier);
        ...
        if (supportTransition) {
			if ((horizontal && txt.width() + widthCorrection <= container.width()+2)||(!horizontal && txt.height() <= container.height()+2)&&!txt.ok) return;
			tools.checkSynchro();
            if (tools.textContainers.indexOf(container) == -1) {
                tools.textContainers.push(container);
				txt.how = {horizontal:horizontal, infinite:infinite, stopOnEnd:stopOnEnd, speed:speed, speedMultiplier:1, customizeSpeed:false};
            ...
            if (horizontal) {
				...
				txt.defaultHTML = txt.html();
                if (infinite && !txt.ok) {
                    txt.html(txt.defaultHTML + (noBreak ? "&nbsp;" : "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"));
                }
            } else {
				txt.defaultHTML = txt.html();
                if (infinite && !txt.ok) {
                    txt.html(txt.defaultHTML + (noBreak ? "<br>": "<br><br>"))
                }
            }
            txt
            ...
            txt.txtSize = txt.txtSize===undefined ? (horizontal ? txt.width() + widthCorrection: txt.height()) : txt.txtSize;
            ...
            if (txt.delta < 0) {
				...
				if(tools.flags.rollStopped)return;
                txt
    ...
    clearTxtValues: function (txt) {
		...
		if(txt.defaultHTML) txt.html(txt.defaultHTML);
		txt.defaultHTML = undefined;
		txt.how = {};
		clearTimeout(tools.checkTimeout);
		tools.flags.checking = false;
    },
	checkSynchro: function(){
		if(tools.flags.checking)return;
		var oldTime;
		function checkTime(){
			function nextCheck(){
				oldTime = time;
				clearTimeout(tools.checkTimeout);
				tools.checkTimeout = setTimeout(checkTime,tools.checkInterval);
			}
			...
			if(time-oldTime>tools.maxTime||window.hidden){
				tools.flags.rollStopped = true;
				tools.stopRollAllTexts();
				nextCheck();
				return;
			}
			if(tools.flags.rollStopped){
				tools.flags.rollStopped = false;
				tools.startRollAllTexts();
			}
			nextCheck();
		}
		tools.flags.checking = true;
		checkTime();
	},
	startRollAllTexts: function(){
		for(var i=0;i<tools.textFields.length;i++){
			tools.rollTextField({container:tools.textContainers[i],txt:tools.textFields[i]},tools.textFields[i].how)
		}
	}
}